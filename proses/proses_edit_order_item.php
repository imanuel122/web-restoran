...
//cek apakah menu sudah ada di list order atau belum
$query = "SELECT id_menu FROM tbl_list_order WHERE id_order = '$kode_order' && id_list_order != '$id_list_order'";
$result = mysqli_query($conn, $query);
$idMenuDbs = [];
while ($idMenuDb = mysqli_fetch_assoc($result)) {
    $idMenuDbs[] = $idMenuDb;
}

for ($i = 0; $i < count($idMenuDbs); $i++) {
    if (in_array($id_menu, $idMenuDbs[$i])) {
        // echo '<script> 
        //     alert("Menu yang di masukkan sudah ada di list order");
...
if (isset($_POST['edit_order_item'])) {
    ...
    $query = "UPDATE tbl_list_order SET `id_order`='$kode_order', `id_menu`='$id_menu', `jumlah`='$jumlah', `catatan`='$catatan' WHERE id_list_order='$id_list_order'";
    $result = mysqli_query($conn, $query);
    if (mysqli_affected_rows($conn) >= 0) {
        // echo '<script> 
        //     alert("Order Item berhasil diedit");
        //     document.location.href = "index.php";
        //     window.location="../?x=orderitem&order=' . $kode_order . '&nama_pelanggan=' . $nama_pelanggan . '&meja=' . $meja . '"; 
        // </script>';
            ...
            <div class="row justify-content-center align-items-center" style="height: 100vh;">
                <div class="col-6 text-center">
                    <div class="alert alert-success d-flex align-items-center justify-content-center" role="alert"
                        style="height: 300px;">
                        <div>
                            <p class="fw-semibold mb-4" style="font-size: 18px;">Success, order item berhasil diedit!</p>
                            <a href="../?x=orderitem&order=<?= $kode_order ?>&nama_pelanggan=<?= $nama_pelanggan ?>&meja=<?= $meja ?>"
            ...
            <div class="row justify-content-center align-items-center" style="height: 100vh;">
                <div class="col-6 text-center">
                    <div class="alert alert-danger d-flex align-items-center justify-content-center" role="alert"
                        style="height: 300px;">
                        <div>
                            <p class="fw-semibold mb-4" style="font-size: 18px;">Gagal, order item gagal diedit!</p>
                            <a href="../?x=orderitem&order=<?= $kode_order ?>&nama_pelanggan=<?= $nama_pelanggan ?>&meja=<?= $meja ?>"
                                class="card-link btn btn-danger">Back</a>
                        </div>
                    </div>
